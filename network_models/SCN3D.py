...
class SCN3D(nn.Module):
    def __init__(self, num_classes=3, in_channels=1, initial_filter_size=64, kernel_size=3, num_downs=3, norm_layer=nn.InstanceNorm3d):
        ...
        unet_block = UnetSkipConnectionBlock(in_channels=in_channels, out_channels=initial_filter_size,
                                             num_classes=num_classes, kernel_size=kernel_size, submodule=unet_block, norm_layer=norm_layer,
                                             outermost=True)
        self.model_unet = unet_block
        ## stage 2 and 3: spatial configuration
        downSampleFactor = 4
        kernalSize = 5
        padSize = int((kernalSize-1)/2)
        channelN = 64
        model_sc = nn.Sequential(
            nn.MaxPool3d(downSampleFactor, stride=downSampleFactor),
            nn.Conv3d(in_channels=num_classes, out_channels=channelN, kernel_size=kernalSize, padding=padSize),
            nn.InstanceNorm3d(channelN),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(in_channels=channelN, out_channels=channelN, kernel_size=kernalSize, padding=padSize),
            nn.InstanceNorm3d(channelN),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(in_channels=channelN, out_channels=channelN, kernel_size=kernalSize, padding=padSize),
            nn.InstanceNorm3d(channelN),
            nn.LeakyReLU(inplace=True),
            nn.Conv3d(in_channels=channelN, out_channels=num_classes, kernel_size=kernalSize, padding=padSize),
            nn.InstanceNorm3d(channelN),
            nn.LeakyReLU(inplace=True),
            nn.ConvTranspose3d(num_classes, num_classes, kernel_size=downSampleFactor, stride=downSampleFactor),
        )
        self.model_sc = model_sc

    def forward(self, x):
        pred_a = self.model_unet(x)
        pred_b = self.model_sc(pred_a)
